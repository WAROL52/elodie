...
export default async function page({}: Props) {
  const emails = await prismaServerSide.emails.findMany({
    include: { User: { include: { email: true } } },
  });
  const users = emails.filter((email) => email.User).map((email) => email.User);
